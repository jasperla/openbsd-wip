...
HL_CLIENT_LIB=${TRUEPREFIX}/lib/xash3d/bshift/libclient.so
HL_SERVER_LIB=${TRUEPREFIX}/lib/xash3d/bshift/libserver.so

LD_LIBRARY_PATH=${XASH_BIN_PATH}:$LD_LIBRARY_PATH ${XASH_BIN_PATH}/xash3d \
	-clientlib ${HL_CLIENT_LIB} -dll ${HL_SERVER_LIB} -game bshift \
	-console $@
