...
def should_comment_file(f, commented_files):
    return (
        ...
        # to us.
        not f.startswith(("share/texmf", "man/", "info/", "share/tlpkg")) or
...
def walk_fake(file_map, commented_files):
    ...
    for root, dirs, files in os.walk(WRKINST):
        for basename in files:
            ...
            unreferenced = False
            if filename == "share/tlpkg/texlive.tlpdb" or \
                    filename.startswith("share/texmf-var/"):
                # Files not in the DB, but belong in the buildset.
...
def process_symlinks(symlink_map):
    ...
    with open("symlinks.mk", "w") as fh:
        fh.write("# $OpenBSD" "$\n")  # split to prevent CVS tag expanding.
