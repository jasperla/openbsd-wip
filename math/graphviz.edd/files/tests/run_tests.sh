...
for t in ${tests}; do
    ...
    # When updating the port, please check if this hack can be removed.
    if [ "${infile}" = "neato-test.nto" ]; then
        echo "WARNING: neato output is non-deterministic, not diffing"
    else
